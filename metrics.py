...
def compute_roc(distances, matches, thresholds, fold_size=10):
    ...
    for fold_index, (training_indices, val_indices) \
        ...
        for threshold_index, threshold in enumerate(thresholds):
            ...
            tp = torch.sum(torch.logical_and(predicts,val_matches)).item()
            fp = torch.sum(torch.logical_and(predicts , ~val_matches)).item()
            tn = torch.sum(torch.logical_and(~predicts , ~val_matches)).item()
            fn = torch.sum(torch.logical_and(~predicts , val_matches)).item()
